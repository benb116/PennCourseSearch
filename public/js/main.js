...
function MenuTriggers() {
    $('li ul li', '#MenuButtons').click(function() { // If the user clicks a Schedule button
        ...
        if (buttonClicked === 'Recolor') {
            colorPalette = JSON.parse(sessionStorage.colorPalette);
            shuffle(colorPalette); // Randomly reorder the colorPalette
            sessionStorage.colorPalette = JSON.stringify(colorPalette);
...
function OtherTriggers() {
    ...
    $('#reqFilter').change(function() { // If the user changes any filters
        var reqCount = $('#reqFilter').val().length; // How many filters are active?
        if (reqCount < 2) {
            var lastreq = [];
            ...
            $('#CourseList li').show();
            var reqs = $('#reqFilter').val();
            // Any class with two reqs will be hidden then shown
            $('.' + reqs[0]).toggle();
            $('.' + reqs[1]).toggle();
        }
    });

    $('#proFilter').change(function() {
        var lastreq = [];
        if (sessionStorage.lastCourseSearch) {
            lastreq = JSON.parse(sessionStorage.lastCourseSearch);
        }
...
function ListScheds(schedInfo, theindex) { // Deal with the list of schedules
    ...
    for (var schedName in schedList) {
        if (schedList.hasOwnProperty(schedName)) {
...
function GetCourseNumbers(search, searchSelect) { // Getting info about courses in a department
    ...
    var requireFilter;
    try {
        requireFilter = '&reqParam=' + ($('#reqFilter').val()[0] || 'noFilter');
    } catch (err) {
        requireFilter = '';
    }
    var programFilter = '&proParam=' + $('#proFilter').val();
    var activityFilter = '&actParam=' + $('#actFilter').val();
    if (requireFilter === '&reqParam=noFilter') {
        requireFilter = '';
    }
    if (programFilter === '&proParam=noFilter') {
        programFilter = '';
    }
    if (activityFilter === '&actParam=noFilter') {
        activityFilter = '';
    }
...
function CourseFormat(JSONRes, passVar) { // Get course number info and display it
    ...
    if (currentReqs) {
        $('#CourseList li').hide();
        $('.' + currentReqs[currentReqs.length - 1]).show();
    }
}
...
function SectionInfoFormat(data, passvar) { // Receive section specific info and display
    ...
    if (data === "No Results") {
        ...
        if (data.reqsFilled.length) {
            HTMLinfo += "Requirements fulfilled: " + data.reqsFilled.join(", ") + "<br><br>";
        }
...
function UpdateSecFilters() { // When a filter value changes, hide and show the relevant sections
    $('#SectionList li').show();
...
function ApplyPCR(courseID, instName) {
    ...
    try {
        pcrFrac = result.cQ / 4;
        if (!isNaN(result.cQ)) {
            var cQStr = result.cQ.toString();
            if (cQStr.length === 1) {
                cQStr += '.';
            } // Format it correctly as '3.00' or '2.50'
...
function ClearSearch() {
    delete sessionStorage.lastCourseSearch;
    $('#CourseList').empty();
    $('#CourseTitle').empty();
    $('#SectionList > ul').empty();
    $('#SectionInfo').empty();
    $('select[multiple] option').attr('selected', false);
    $('#closedCheck').prop('checked', true);
    $('#actFilter, #proFilter').val("noFilter");
    $('#CSearch').val('');
}
