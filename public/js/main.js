  ...
  $( 'li ul li', '#MenuButtons' ).click( function () { // If the user clicks a Schedule button
    ...
    if ( buttonClicked == 'New' ) {
      sweetAlert( {
        ...
        if ( inputValue === false )
          return false;else if ( inputValue === "" ) {
...
function OtherTriggers() {
  ...
  $( '#reqFilter').change( function () { // If the user changes any filters
    var reqCount = $('#reqFilter').val().length;
    if (reqCount < 2) {
      var lastreq = [];
      if (sessionStorage.lastCourseSearch) {lastreq = JSON.parse( sessionStorage.lastCourseSearch );}
...
function getCourseNumbers( search, searchSelect ) { // Getting info about courses in a department
  ...
  sessionStorage.lastCourseSearch = JSON.stringify( [
    search,
    searchSelect
  ] );
...
function CourseFormat( JSONRes, passVar ) { // Get course number info and display it
  ...
  $( '.tooltip', '#CourseList' ).tooltipster( { position: 'right' } );
...
function FormatSectionsList( courseInfo, suppress ) { // Receive section and star info and display them
  ...
  if ( courseInfo[ 1 ] == 'No Results' ) {
    ...
    UpdatePlusCancel();
    $( '.tooltip', '#SectionList' ).tooltipster( { position: 'right' } );
...
function SpitSched( courseSched ) {
  ...
  if ( incSun == 1 ) {
    weekdays.unshift( 'U' ); fullWeekdays.unshift( 'Sunday' );
  } // Update weekdays array if necessary
  if ( incSat == 1 ) {
    weekdays.push( 'S' ); fullWeekdays.push( 'Saturday' );
  }
