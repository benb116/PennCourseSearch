...
var currentTerm = '2019C';
var currentRev = '2018CRev';
...
var rpm = (process.argv[3] || 95);
var opendata = require('../opendata.js')(rpm);
var lastRequestTime = 0;

var source = process.argv[2].toLowerCase(); // registrar or review
var index = Number(process.argv[4]); // Can be a number or a dept code
var endindex;
try {
    endindex = Number(process.argv[5]) + 1; // If there's a second number, end at that number
} catch(err) {}
var limit = false;
...
function PullRegistrar(thedept) {
    ...
    lastRequestTime = opendata.RateLimitReq(baseURL, 'registrar', {}, lastRequestTime, 1);
}
...
function PullReview(index) {
    ...
    // Get raw data
        ...
        var deptReviews = {};
        try {
            deptReviews = JSON.parse(body).result.values;
        } catch(err) {
            console.log(thedept);
            console.log(err);
            return;
