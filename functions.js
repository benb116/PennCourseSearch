$(document).ready(function () {
    ...
    var subtitles = ["Free shipping on all items in your course cart", "You can press the back button, but you don't even need to.", "Invented by Benjamin Franklin in 1793", "Faster than you can say 'Wawa run'", "Classes sine PennCourseSearch vanae",
    // "On PennCourseSearch, no one knows you're Amy G.",
    "Designed by Ben in Speakman. Assembled in China.", "Help! I'm trapped in a NodeJS server! Bring Chipotle!", "With white sauce AND hot sauce", "Now 3.9% faster", "Number of squirrels online: 6", "Handling the business side since 2014", "Actually in touch"];
    var paymentNoteBase = "https://venmo.com/?txn=pay&recipients=BenBernstein&amount=1&share=f&audience=friends&note=";
    var paymentNotes = ["PennCourseSearch%20rocks%20my%20socks!", "Donation%20to%20PennInTouch%20Sucks,%20Inc.", "For%20your%20next%20trip%20to%20Wawa", "Offsetting%20the%20increased%20price%20of%20chicken%20over%20rice"];
    ...
    if (/Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        // Doesn't look good on mobile, so tell the user
        setTimeout(function () {
            sweetAlert({
                title: 'PCS Alert',
        ...
        $.get('/Status').done(function (statusMessage) {
            if (statusMessage !== 'hakol beseder') {
                // If there is a status message from the server, it will be passed in the #StatusMessage block
                setTimeout(function () {
                    sweetAlert({
                        title: 'PCS Alert',
                        html: true,
...
function Uniquify(str, arr) {
    ...
    if (arr.indexOf(str) === -1) {
        ...
        // If it does, then we "+1" the string
        var lastchar = str[str.length - 1];
        if (isNaN(lastchar) || str[str.length - 2] !== ' ') {
            // e.g. if string == 'schedule' or 'ABC123'
...
function FormatID(rawParam) {
    ...
    if (isFinite(searchParam[2])) {
        // If the third character is a number (e.g. BE100)
        splitTerms(2);
    } else if (isFinite(searchParam[3])) {
        // If the fourth character is a number (e.g. CIS110)
        splitTerms(3);
    } else if (isFinite(searchParam[4])) {
        // If the fifth character is a number (e.g. MEAM110)
        splitTerms(4);
    } else {
...
function GenMeetBlocks(sec) {
    var blocks = [];
    for (var day in sec.meetDay) {
        if (sec.meetDay.hasOwnProperty(day)) {
